...
System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
        ...
        mbDeactivateLocalizationMode(false), mDeviceType((torch::cuda::is_available()) ? c10::kCUDA : c10::kCPU),
        mDevice(c10::Device(mDeviceType))
{
    ...
    if(mSensor==MONOCULAR)
        ...
        cout << "RGB-D" << endl;
    else if(mSensor==SP_MONOCULAR)
    {
        cout << "SuperPoint Monocular" << endl;
        cout << "\n SuperPoint model is loading Weights...\n";
        /* SuperPoint model loading */
        mpSPModel = std::make_shared<SuperPointSLAM::SuperPoint>();
        torch::load(mpSPModel, mSPWeightPath);

        /* This option should be done exactly as below */
        mTensorOptions = c10::TensorOptions()
                        .dtype(torch::kFloat32)
                        .layout(c10::kStrided)
                        .requires_grad(false);

        if (mDeviceType == c10::kCUDA)
            mpSPModel->to(mDevice);
        mpSPModel->eval();
...
cv::Mat System::TrackSPMonocular(const cv::Mat &im, const double &timestamp)
{
    if(mSensor!=SP_MONOCULAR)
    {
        cerr << "ERROR: you called TrackMonocular but input sensor was not set to Monocular." << endl;
